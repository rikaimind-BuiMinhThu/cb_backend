class Api::V1::Managements::EmailsController < ApplicationController
  ...
  def show
    ...
    return render json: {code: 2, data: "No permission"} if current_user.admin_client? && email.user != current_user
  ...
  def email_params
    params.require(:email).merge!(user_id: current_user.id).permit(:email_template_name, :sender_name, :to, :reply_to, :subject, :content, :user_id)
  end
end
